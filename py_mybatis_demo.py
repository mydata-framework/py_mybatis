...
def test2():
    sqlSession = pyMybatis.getSqlSession()
    ...
    insert = pyMybatis.insert(sqlSession=sqlSession, statementId='UserMapper.insertUser',
                              dictParam={'username': 'liutao 007'},
                              keyProperty='id')
    print(insert)

    update = pyMybatis.update(sqlSession=sqlSession, statementId="UserMapper.updateUser",
                              dictParam={'username': 'liutao 008', 'id': insert['id']}
                              )
    print(update)
    ...
    page = pyMybatis.selectPage(sqlSession=sqlSession, countStatementId='selectPage_count',
                                listStatementId='selectPage_list',
                                pageNum=1,
                                pageSize=10)
    print(page)
...
def test3():
    with pyMybatis.getTransactionedSqlSession() as sqlSession:
        insert = pyMybatis.insert(sqlSession=sqlSession,
